...
public class EwrapperTest extends TestCase {
    ...
    protected void setUp() throws Exception {
		...
		testData = new OpenDaTestSupport(EwrapperTest.class, "model_Themi");
		...
		MLDouble mlDouble = new MLDouble( "E", E, 64 ); // Store test in 3 rows.
	...
	public void testInitialize() {
		...
		for (int item = 0; item < exchangeItems.length; item++) {
			if (exchangeItems[item].getId().equalsIgnoreCase(expectedIDs[item])) {
				...
				assertEquals(expectedE.length,readValues.length);
				assertEquals(readTimes.length,readValues.length);
				for (int i = 0; i<expectedTime.length; i++) {
					assertEquals(expectedTime[i],readTimes[i]);
					assertEquals(expectedE[i],readValues[i]);
				}
